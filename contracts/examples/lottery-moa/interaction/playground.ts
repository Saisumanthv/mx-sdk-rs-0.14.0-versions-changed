...
import { Account, Argument, BackendSigner, Balance, Code, ContractFunction, GasLimit, NetworkConfig, ProxyProvider, SmartContract } from "@dharitrinetwork/erdjs";
import { AbiRegistry, U32Value } from "@dharitrinetwork/erdjs/out/smartcontracts/typesystem";
import { BinaryCodec } from "@dharitrinetwork/erdjs/out/smartcontracts/codec"
...
async function main() {
    ...
    let namespace = abi.findNamespace("lottery-moa");

    await NetworkConfig.getDefault().sync(provider);
    await user.sync(provider);

    // Deploy TRANSACTION
    let contractFile = path.resolve(__dirname, "../output/lottery-moa.wasm");
    ...
    let transactionStart = contract.call({
        ...
        args: [
            ...
            Argument.fromBigInt(Balance.MOA(1).valueOf()),
